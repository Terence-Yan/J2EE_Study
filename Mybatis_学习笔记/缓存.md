...
4.注意事项：
   a).对应的POJO类要实现序列化接口；
   b).查询出的数据都会被默认首先保存在一级缓存中(一级缓存是默认开启的)，只有在会话关闭或提交以后，一级缓存中的数据才会更新到二级缓存。
...
5.localCacheScope：本地缓存作用域(3.3版本及以后新增的属性)，该属性有两个取值：SESSION(使用一级缓存)与STATEMENT(禁用一级缓存)。
...
2.整合第三方缓存方案的步骤：
  a).引入第三方缓存的jar包；
  b).导入与选择的第三方缓存整合的适配jar包，参见 Mybatis的github官网https://github.com/mybatis；
